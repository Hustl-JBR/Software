...
export default async function StagingWorkspace({
  params,
  searchParams,
}: {
  ...
  return (
    <main className="staging-workspace">
      <header className="panel-heading">
        <div>
          <p className="overline">Persistent staging</p>
          <h1>{membership.organization.name} operations</h1>
          <p className="muted">
            Signed in as {membership.user.name}. All records below are
            PostgreSQL-backed and organization-scoped.
          </p>
        </div>
        <Link className="text-button" href={`/org/${slug}`}>
          Intake queue
        </Link>
      </header>
      ...
      {status.error && (
        <div className="alert error" role="alert">
          Operation blocked: {status.error}
        </div>
      )}
        ...
        <div className="load-table">
          {members.map((item) => (
            <div className="table-row" key={item.id}>
              ...
              <span>
                {Array.from(
                  new Set([item.role, ...item.roles.map((role) => role.role)]),
                ).join(" + ")}
              </span>
            </div>
          ))}
      ...
      <div className="workspace-grid">
        ...
        >
          ...
          <input
            name="cargoCoverageCents"
            type="number"
            min="0"
            placeholder="Cargo coverage cents"
          />
          <input
            name="quotedCostCents"
            type="number"
            min="0"
            placeholder="Carrier cost cents"
          />
        </FormCard>
          ...
          <article className="card" key={request.id}>
            ...
            {request.quotes.map((quote) => (
              <div className="table-row" key={quote.id}>
                <strong>${(Number(quote.amountCents) / 100).toFixed(2)}</strong>
        ...
        {loads.map((load) => (
          <article className="card" key={load.id}>
            ...
            {load.trackingUpdates.map((update) => (
              <p key={update.id}>
                Tracking · {update.status} · {update.location ?? "No location"}
              </p>
            ))}
            {load.communications.map((item) => (
              <p key={item.id}>
                {item.channel} · {item.partyType} · {item.summary}
              </p>
            ))}
          </article>
        ))}
...
function Hidden({ slug }: { slug: string }) {
  return <input type="hidden" name="organizationSlug" value={slug} />;
}
